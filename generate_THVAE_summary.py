...
from thvae.data_pipelines.assemblers.sum_pipeline import assemble_sum_pipeline
...
from thvae.utils.helpers.run import gen_seqs, summ_eval, gen_summs
...
# parser.add_argument('--arch_instance', type=str, default='res_bnelu',
#                         help='path to the architecture instance')
...
gen_data_sources = [
    {"data_path": run_hp.gen_add4, 'early_term': run_hp.gener_early_term}
]
...
start_id = bart_tokenizer._convert_token_to_id("<s>")
end_id = bart_tokenizer._convert_token_to_id("</s>")
pad_id = bart_tokenizer._convert_token_to_id("<pad>")

arch_instance = utils.get_arch_cells(parser_args.arch_instance)

model = Model(arch_instance,  **model_hp.to_dict())
...
data_chunk_iter = train_pipeline.iter(**train_data_source)

# num_total_iter= sum(1 for _ in data_chunk_iter) * run_hp.epochs
num_total_iter = 10

imodel = IModel(model=model, grads_clip=run_hp.grads_clip,
                device=run_hp.device, min_gen_seq_len=run_hp.min_gen_seq_len,
                learning_rate=run_hp.learning_rate,
                kl_anneal_portion=run_hp.kl_anneal_portion,
                kl_const_portion=run_hp.kl_const_portion,
                kl_const_coeff=run_hp.kl_const_coeff,
                num_total_iter=num_total_iter,
                num_latent_scales=model.num_latent_scales,
                groups_per_scale=model.groups_per_scale,
                weight_decay_norm_anneal=run_hp.weight_decay_norm_anneal,
                weight_decay_norm_init=run_hp.weight_decay_norm_init,
                weight_decay_norm=run_hp.weight_decay_norm,
                z_kl_ann=z_kl_ann,
                beamer=beamer)

idev = IDev(imodel=imodel, train_data_pipeline=train_pipeline,
            val_data_pipeline=val_pipeline, gen_sum_pipeline=summary_pipeline, word_vocab=word_vocab,
...
if parser_args.infer_input_file_path is None:
    ...
    data_iter = infer_data_pipeline.iter(data_path=infer_inp_file_path)
    output_file_path = infer_out_file_path
    summ_gen_func = partial(idev.summ_generator,
                             summ_post_proc=summ_pproc) 
